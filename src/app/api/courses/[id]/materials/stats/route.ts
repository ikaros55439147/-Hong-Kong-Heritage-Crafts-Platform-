...
/**
 * GET /api/courses/[id]/materials/stats - Get learning material statistics for a course
 */
export async function GET(
  ...
  try {
    ...
    const stats = await learningMaterialService.getCourseMaterialStats(
      params.id,
      craftsmanProfile.id
    )
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch material stats'
      },
      { status: 400 }
    )
  }
